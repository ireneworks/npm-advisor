...
export default function Navigation() {
  ...
  const { data, error } = useNpmSWR<INpmSearchResponse>(searchKey);
  ...
  const onClickAutofill = useCallback(
    (e: React.MouseEvent, packageName: string) => {
      e.preventDefault();
      setQuery("");
  ...
  return (
    <div className="flex gap-5 py-5 px-12 items-center">
      <h1
              ...
              {data ? (
                data.objects.map((item: INpmPackage) => (
                  <div
                    ...
                    onMouseDown={(e) => onClickAutofill(e, item.package.name)}
                  >
